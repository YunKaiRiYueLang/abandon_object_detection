  ...
  sensitivity(72), bgThreshold(162), learningRate(49), noiseVariance(195)
{
  ...
  setup("./config/LBFuzzyGaussian.xml");/*配置文件的载入和保存*/
  //std::cout<<sensitivity<<"  "<<bgThreshold<<" "<<learningRate<<" "<<noiseVariance<<std::endl;
}
...
// 用m_pbgmodel 设置参数 更新模型,拿出更新后的前景和背景
void LBFuzzyGaussian::process(const cv::Mat &img_input, cv::Mat &img_output, cv::Mat &img_bgmodel)
{
  ...
  if (firstTime)
  {
    ...
    m_pBGModel->InitModel(frame);/*用第一帧初始化模型*/
  }
